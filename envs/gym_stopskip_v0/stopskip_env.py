...
class StopSkipEnv(gym.Env):
    ...
    def reset_solution(self):
        ...
        self.exp_direction = StopSkipEnvDirection.Indeterministic
    ...
    def execute(self, action):
        ...
        if len(self.exp_alignment) > 0 and self.exp_alignment[-1] == action:
            return False

        if sum(1 for node in self.exp_alignment if node == action) > 1:
            return False

        if self.exp_direction == StopSkipEnvDirection.Indeterministic:
            if len(self.exp_alignment) != 0 and action == self.exp_alignment[-1]:
                return False

            self.exp_alignment.append(action)

            if len(self.exp_alignment) == 2:
                self.exp_direction = (StopSkipEnvDirection.Forward
                                      if self.exp_alignment[1] > self.exp_alignment[0]
                                      else StopSkipEnvDirection.Backward)
        else:
            direction = (StopSkipEnvDirection.Forward
                         if action > self.exp_alignment[-1]
                         else StopSkipEnvDirection.Backward)

            if direction == self.exp_direction:
                self.exp_alignment.append(action)
            elif self.exp_direction == StopSkipEnvDirection.Forward:
                self.exp_alignment.append(action)
                self.exp_direction = StopSkipEnvDirection.Backward
